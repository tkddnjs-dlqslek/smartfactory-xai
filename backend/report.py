...
def _fallback(ctx: dict, tone: str) -> str:
    ...
    if not top or ctx.get("status") == "NORMAL":
        return "원인: 이상 없음. 대처: 현재 설정 유지. (LLM 미연결 — 템플릿)"
    s0 = top[0]
    return (f"원인: {_ko(s0['name'])} {s0['sigma']} 비정상. "
